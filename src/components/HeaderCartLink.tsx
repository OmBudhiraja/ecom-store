...
import { type UserCart } from "~/server/api/cart";
...
function HeaderCartLink({
  ...
  return (
    <Link href="/cart" className="relative px-2 py-1">
      <CartIcon size={26} />
      {cart && cart.length > 0 && (
        <span className="absolute -right-1 -top-2 flex h-6 w-6 items-center justify-center rounded-full bg-zinc-900 p-1 text-sm text-white">
          {cart.length}
        </span>
      )}
    </Link>
